const translations = {
  zh: {
    ...
    hero_lede: "KeyAuth 是一款为 iPhone 和 iPad 打造的 TOTP 验证器。你的密钥在本地生成、加密和使用；云端只负责同步密文。",
    ...
    step_sync_text: "需要跨设备时，由 iCloud 同步加密数据；本地使用不依赖网络。",
    recovery_eyebrow: "自主选择的恢复",
    recovery_title: "换一台设备，<br /><em>不必交出控制权。</em>",
    recovery_text: "启用跨设备恢复后，KeyAuth 会把主密钥环用独立 Recovery Key 包装。iCloud 保存的仍然只是加密恢复包，真正的恢复密钥只在你手里。",
    recovery_cta: "了解恢复边界",
    ...
    faq_device_question: "换设备需要什么？",
    faq_device_answer: "在旧设备上启用恢复并妥善保存独立 Recovery Key，然后在新设备上完成恢复。无论是否恢复，打开保险库仍需要 Face ID 或设备密码。",
    faq_supported_question: "KeyAuth 支持哪些设备？",
    faq_supported_answer: "当前产品面向 iOS 26 或更高版本的 iPhone 和 iPad。首次使用需要完成系统设备认证。",
    download_title: "把验证码，<br /><em>放回你手里。</em>",
    download_text: "一个更安静、更透明的验证器。为 iPhone 和 iPad 而做。",
    download_cta: "查看开源项目",
    ...
    privacy_recovery_title: "跨设备恢复",
    privacy_recovery_text: "如果你主动启用恢复，KeyAuth 会用独立 Recovery Key 包装主密钥环，并把加密恢复包放入 CloudKit。Recovery Key 不会上传，必须由你自行保存。",
    ...
    privacy_choices_text: "你可以不启用 iCloud 同步或跨设备恢复，继续把 KeyAuth 当作本地验证器使用。删除账户会先从本地移除并排队同步删除；卸载 App 不会自动删除 CloudKit 中已有记录。",
  ...
  en: {
    ...
    nav_recovery: "Cross-device recovery",
    nav_faq: "FAQ",
    nav_privacy: "Privacy",
    ...
    hero_lede: "KeyAuth is a TOTP authenticator for iPhone and iPad. Your secrets are generated, encrypted, and used on-device; the cloud only syncs ciphertext.",
    ...
    step_sync_text: "When you need another device, iCloud syncs encrypted data; local use never depends on the network.",
    recovery_eyebrow: "RECOVERY, BY CHOICE",
    recovery_title: "Move to another device,<br /><em>without giving up control.</em>",
    recovery_text: "When cross-device recovery is enabled, KeyAuth wraps the master-key ring with an independent Recovery Key. iCloud still stores only an encrypted envelope; the recovery key stays with you.",
    ...
    faq_device_question: "What do I need to move to another device?",
    faq_device_answer: "Enable recovery on the old device and keep the independent Recovery Key safe, then restore on the new device. Opening the vault still requires Face ID or your passcode.",
    faq_supported_question: "Which devices does KeyAuth support?",
    faq_supported_answer: "KeyAuth currently targets iPhone and iPad running iOS 26 or later. The first launch requires system device authentication.",
    download_title: "Put your codes,<br /><em>back in your hands.</em>",
    download_text: "A quieter, more transparent authenticator. Made for iPhone and iPad.",
    ...
    privacy_recovery_title: "Cross-device recovery",
    privacy_recovery_text: "If you choose to enable recovery, KeyAuth wraps the master-key ring with an independent Recovery Key and stores only the encrypted envelope in CloudKit. The Recovery Key is never uploaded and must be kept by you.",
    ...
    privacy_choices_text: "You can leave iCloud sync and cross-device recovery disabled and use KeyAuth as a local authenticator. Deleting an account removes it locally first and queues the cloud deletion; uninstalling the app does not automatically delete existing CloudKit records.",
...
const pageMetadata = {
  home: {
    zh: {
      ...
      description: "KeyAuth 是一款本地优先、端到端加密的 iOS TOTP 验证器。验证码留在你的掌控中。",
      ogTitle: "KeyAuth — 验证码，回到你自己的掌控中",
      ogDescription: "本地优先的 TOTP 验证器。AES-256-GCM 加密、Face ID 保护，iCloud 只同步密文。"
    },
    en: {
      title: "KeyAuth — Your codes, your control",
      description: "KeyAuth is a local-first, end-to-end encrypted TOTP authenticator for iOS. Your codes stay in your hands.",
      ogTitle: "KeyAuth — Your codes, back in your hands",
      ogDescription: "A local-first TOTP authenticator with AES-256-GCM encryption, Face ID protection, and ciphertext-only iCloud sync."
    }
  },
  privacy: {
    zh: {
